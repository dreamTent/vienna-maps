#!/usr/bin/env python3
"""
Download Vienna oblique 2020 pyramid tiles without stitching (mirrors server paths).
...
# Oblique_Wien_2020 — see oblique-viewer.html / Stadtplan3D data source
REMOTE_BASE_URL = (
    "https://www.wien.gv.at/stadtplan3d/datasource-data/Oblique/Oblique_Wien_2020"
)
DEFAULT_TILES_DIR = Path("sources/2020/tiles")
TILE_SIZE = 1024
# Defaults if a row omits width/height (2020 sheet size; 2023 used 14144×10560)
DEFAULT_WIDTH_FALLBACK = 7700
DEFAULT_HEIGHT_FALLBACK = 10300
...
def iter_images_from_tile(tile_file: Path) -> Iterable[dict]:
    ...
    for row in rows[1:]:
        ...
        yield {
            "name": row[col["name"]],
            "width": int(row[col["width"]] or DEFAULT_WIDTH_FALLBACK),
            "height": int(row[col["height"]] or DEFAULT_HEIGHT_FALLBACK),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Download 2020 oblique JPEG tiles (one or all pyramid levels), mirroring server paths."
    )
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("oblique_2020_tiles"),
        help="Root folder for mirrored tile tree (default: ./oblique_2020_tiles)",
    )
    p.add_argument(
        "--tiles-dir",
        type=Path,
        default=DEFAULT_TILES_DIR,
        help="Directory with 2020 tile index JSON files",
    )
    p.add_argument(
        "--zoom-level",
        type=int,
...
def main() -> None:
    ...
    failed_downloads: List[Tuple[str, str]] = []  # (url, result message)
            ...
            with ThreadPoolExecutor(max_workers=args.workers) as ex:
                ...
                for fut in as_completed(futs):
                    ...
                    if res == "ok":
                        ...
                        z_fail += 1
                        url, _ = futs[fut]
                        failed_downloads.append((url, res))
                        print(f"  {res} — {url}")
    ...
    if failed_downloads:
        fail_log = out_root / "failed_downloads.txt"
        fail_log.write_text(
            "\n".join(f"{msg} — {url}" for url, msg in failed_downloads) + "\n",
            encoding="utf-8",
        )
        print(f"\nFailed downloads ({len(failed_downloads)}):")
        for url, msg in failed_downloads:
            print(f"  {msg} — {url}")
        print(f"Saved failed list to: {fail_log.resolve()}")
    if total_fail:
        sys.exit(1)
